  ...
  export default {
    ...
    created () {
      if (!localStorage.signedIn) {
        ...
        securedAxiosInstance.get('/api/v1/records')
          .then(response => { this.records = response.data })
          .catch(error => this.setError(error, 'Something went wrong'))
  
        securedAxiosInstance.get('/api/v1/artists')
    ...
    methods: {
      ...
      addRecord () {
        ...
        securedAxiosInstance.post('/api/v1/records/', { record: { title: this.newRecord.title, year: this.newRecord.year, artist_id: this.newRecord.artist } })
      ...
      removeRecord (record) {
        securedAxiosInstance.delete(`/api/v1/records/${record.id}`)
          .then(response => {
      ...
      updateRecord (record) {
        this.editedRecord = ''
        securedAxiosInstance.patch(`/api/v1/records/${record.id}`, { record: { title: record.title, year: record.year, artist_id: record.artist } })
