  ...
  export default {
    ...
    methods: {
      ...
      addArtist () {
        ...
        securedAxiosInstance.post('/api/v1/artists/', { artist: { name: this.newArtist.name } })
      ...
      removeArtist (artist) {
        securedAxiosInstance.delete(`/api/v1/artists/${artist.id}`)
          .then(response => {
      ...
      updateArtist (artist) {
        this.editedArtist = ''
        securedAxiosInstance.patch(`/api/v1/artists/${artist.id}`, { artist: { title: artist.name } })
